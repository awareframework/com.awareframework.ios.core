...
private enum SyncState: Equatable {
    ...
    static func == (lhs: SyncState, rhs: SyncState) -> Bool {
        switch (lhs, rhs) {
        case (.idle, .idle), (.active, .active), (.cancelling, .cancelling),
            (.completed, .completed):
            return true
...
open class DbSyncHelper: NSObject {
    ...
    private let sessionQueue = DispatchQueue(
        label: DbSyncConstants.sessionQueueLabel, qos: .utility)
    ...
    private func shouldStopRetrying() -> Bool {
        ...
        let maxRetries = 10
        let currentRetries = UserDefaults.standard.integer(
            forKey: "aware.sync.retries.\(tableName)")
        let shouldStop = currentRetries >= maxRetries

        logTrace(
            "shouldStopRetrying() currentRetries: \(currentRetries)/\(maxRetries), shouldStop: \(shouldStop)"
        )
    ...
    private func performDataSync() {
        ...
        if originalNumOfCandidates == 0 {
            ...
            if config.debug {
                logVerbose(
                    "→ performDataSync() originalNumOfCandidates: \(originalNumOfCandidates), reset totalUploadedRecords"
                )
            }
        }

        // Get target data for synchronization
        let candidates = getUploadCandidates(
            lastUploadedId: lastUploadedId, limit: config.batchSize)

        if config.debug {
            logVerbose(
                "→ performDataSync() found \(candidates.count) candidates, dispatching to upload")
        }
     ...
     */
    open func getUploadCandidates(lastUploadedId: Int64, limit: Int) -> [[String: Any]] {
     ...
     */
    private func buildRequestBody(from candidates: [[String: Any]]) throws -> String {
        let deviceId = AwareUtils.getCommonDeviceId()
        let uploadCandidates = candidates.map { candidate -> [String: Any] in
            guard config.serverType == .aware_light else {
                return candidate
            }
            var uploadCandidate = candidate
            uploadCandidate.removeValue(forKey: "id")
        ...
        if self.config.compactDataFormat {
            ...
            payloadData = aggregatedData
        } else {
            // Standard format (row by row)
            payloadData = uploadCandidates
        }

        let jsonData = try JSONSerialization.data(withJSONObject: payloadData, options: [])
        let data = String(data: jsonData, encoding: .utf8)!

        var requestStr: String
        if usesNewAwareServerEndpoint {
            var requestParams: [String: Any] = [
                "deviceId": deviceId,
                "data": payloadData,
                "tableName": tableName,
            ]
            requestParams["timestamp"] = Int64(Date().timeIntervalSince1970 * 1000)
            let requestData = try JSONSerialization.data(withJSONObject: requestParams, options: [])
            requestStr = String(data: requestData, encoding: .utf8)!
        } else {
            // Build request body in URL-encoded format for legacy AWARE server.
            requestStr = buildFormRequestBody([
                "device_id": deviceId,
                "data": data,
            ])
        }
    ...
    private func buildFormRequestBody(_ parameters: [String: String]) -> String {
    ...
    private func buildHTTPRequest(with requestBody: String) -> URLRequest? {
        // Send HTTP request
        var hostName = AwareUtils.cleanHostName(self.host)
        while hostName.hasSuffix("/") {
            hostName = String(hostName.dropLast())
        }
        if config.serverType == .aware && hostName.hasSuffix("/index.php") {
            hostName = String(hostName.dropLast("/index.php".count))
        }

        let urlString: String
        if config.serverType == .aware_micro {
            let safeStudyKey =
                config.studyKey.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed)
                ?? config.studyKey
            urlString = "https://\(hostName)/api/\(config.studyNumber)/\(safeStudyKey)/insert/"
        } else if usesNewAwareServerEndpoint {
            urlString = "https://\(hostName)/insert/"
        } else {
            // Encode table name to URL-safe format
            let safeTableName =
                self.tableName.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed)
                ?? self.tableName
            let safeStudyKey =
                config.studyKey.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed)
                ?? config.studyKey
            urlString =
                "https://\(hostName)/index.php/webservice/index/\(config.studyNumber)/\(safeStudyKey)/\(safeTableName)/insert"
        }
    ...
    private var usesNewAwareServerEndpoint: Bool {
        switch config.serverType {
        case .aware_micro, .aware_light:
            return true
        case .none, .aware:
            return false
        }
    ...
    private func handleNextBatch() {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + DbSyncConstants.nextBatchDelay)
        { [weak self] in
            ...
            if let pCallback = self?.config.progressHandler {
                ...
                if self?.config.debug == true {
                    ...
                    self?.logVerbose(
                        "Progress: \(uploadedItems)/\(totalItems) records (\(percentage)%)")
                }
     ...
     */
    open func upload(_ candidates: [[String: Any]]) {
        if config.debug {
        ...
        do {
            ...
            if let request = buildHTTPRequest(with: requestBody) {
                ...
                task.taskDescription =
                    "Upload[\(tableName)]_Batch[\(candidates.count)]_LastID[\(idOfLastCandidate ?? 0)]"
    ...
    private func createURLSession() -> URLSession {
        ...
        if self.config.backgroundSession {
            ...
            sessionConfig = URLSessionConfiguration.background(
                withIdentifier: "\(DbSyncConstants.backgroundSessionIdentifierPrefix).\(tableName)")
        } else {
            // Normal session
    ...
    open func urlSession(
        ...
        if let httpResponse = response as? HTTPURLResponse {
            if DbSyncConstants.httpSuccessRange.contains(httpResponse.statusCode) {
                ...
                logWarning(
                    "HTTP error response: \(httpResponse.statusCode) from \(httpResponse.url?.absoluteString ?? "unknown URL")"
                )
     ...
     */
    private func evaluateResponseState(
        error: Error?, task: URLSessionTask? = nil, session: URLSession? = nil
    ) -> Bool {
        if let unwrappedError = error {
            ...
            if nsError.domain == NSURLErrorDomain {
                ...
                case NSURLErrorCancelled:
                    logWarning(
                        "Request was cancelled - Check server response status or network interruption\(sessionContext)"
                    )
                case NSURLErrorTimedOut:
                    logError(
                        "Request timed out - Server may be overloaded or network is slow\(sessionContext)"
                    )
                case NSURLErrorNetworkConnectionLost:
                    logError(
                        "Network connection lost - Check WiFi/cellular connectivity\(sessionContext)"
                    )
                case NSURLErrorNotConnectedToInternet:
                    logError(
                        "No internet connection - Verify network settings and connectivity\(sessionContext)"
                    )
                case NSURLErrorCannotFindHost:
                    logError("Cannot find host - Verify server URL: \(host)\(sessionContext)")
                case NSURLErrorCannotConnectToHost:
                    logError(
                        "Cannot connect to host - Server may be down or unreachable\(sessionContext)"
                    )
                ...
                case NSURLErrorSecureConnectionFailed:
                    logError(
                        "SSL/TLS connection failed - Check certificate validity\(sessionContext)")
                default:
                    logError(
                        "Network error (code: \(nsError.code)) - \(nsError.localizedDescription)\(sessionContext)"
                    )
                }
            } else {
    ...
    private func updateLastUploadedId() {
        if let candidateId = idOfLastCandidate {
            ...
            if config.debug {
                logVerbose(
                    "Regist ID of the last upload object => \(candidateId) : \(lastUploadedId) ~ \(candidateId)"
                )
            }
        }
    }
    ...
    public func stopGracefully(
        timeout: TimeInterval = 30.0, handler: ((_ completed: Bool) -> Void)? = nil
    ) {
        guard canStop() else {
